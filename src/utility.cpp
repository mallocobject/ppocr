...
namespace PaddleOCR
{
	...
	void Utility::print_result(const std::vector<OCRPredictResult>& ocr_result, QStringList& res) noexcept
	{
		for (size_t i = 0; i < ocr_result.size(); ++i)
		{
			...
			if (ocr_result[i].score != -1.0)
			{
				str += QString("rec text: ");
				str += QString::fromStdString(ocr_result[i].text);
			}

			// cls
			if (ocr_result[i].cls_label != -1)
			{
				str += QString("cls label: %1")
					.arg(ocr_result[i].cls_label);
			}
			res.append(str);
		}
	}
